...
TOKEN = '8059842619:AAG7HetgrAw7GWq5sg0h3p24OYZCASuHHT0'
ADMIN_ID =  7683046483 # ايديك
YOUR_USERNAME = '@ZX_HAU'  #  @ يوزرك مع
CHANNEL_USERNAME = '@DG_ISV'  # يوزر القناة الإجبارية
...
def ask_to_upload_file(call):
    ...
    if bot_locked:
        bot.send_message(call.message.chat.id, "⚠️ البوت مقفل حالياً. الرجاء التواصل مع المطور @DG_ISV.")
        return
    ...
    if free_mode or (user_id in user_subscriptions and user_subscriptions[user_id]['expiry'] > datetime.now()):
        ...
        bot.send_message(call.message.chat.id, "⚠️ يجب عليك الاشتراك لاستخدام هذه الميزة. الرجاء التواصل مع المطور @DG_ISV .")
...
def handle_file(message):
    ...
    if bot_locked:
        bot.reply_to(message, "⚠️ البوت مقفل حالياً. الرجاء التواصل مع المطور @DG_ISV.")
        return
    ...
    if free_mode or (user_id in user_subscriptions and user_subscriptions[user_id]['expiry'] > datetime.now()):
        ...
        bot.reply_to(message, "⚠️ يجب عليك الاشتراك لاستخدام هذه الميزة. الرجاء التواصل مع المطور @DG_ISV.")
